...
var DatabaseAccessService = function( options ) {
    ...
    var createPrivateChannel = function( request, callback ) {
        ...
        // memory leak! should add a timeout to disconnect and remove listeners after client is closed
        consumer.onMessage( function( msg ) {
            if ( msg.ssid !== ssid && msg.message && msg.message.cmd ) {
                log.info('received command message: ', msg);
                dash.defer( service.commandMessageHandler, consumer, msg );
            }
        } );
    ...
    this.commandMessageHandler = function(consumer, msg) {
        var rid = msg.message.rid,
            cmd = dash.clone( msg.message.cmd );

        log.info( 'db command message: ', msg );

        dao.execute( cmd, function(err, result) {
            var response = { };
...
DatabaseAccessService.createInstance = function( config ) {
    ...
    var createDao = function(name) {
        var Dao = require('../dao/' + name ),
            opts = {};

        opts.log = logManager.createLogger( name );
    ...
    var createDatabaseService = function() {
        ...
        opts.dao = createDao( config.daoName );
